...
class User
{
   ...
   public function __construct($user=null)
   {
      ...
      if(!$user)
      {
         if(Session::exits($this->_sessionName))
         {
   ...
   public function login($username=null,$password=null,$remember=false,$notgroup=0)
   {
    if(!$username&&!$password&&$this->exits())
    {
    ...
    else
    {
      ...
      if($user)
      {
         if($this->data()->password===Hash::make($password,$this->data()->salt)&& $this->data()->group_id != $notgroup)
         {
            Session::put($this->_sessionName,$this->data()->id);
            if($remember)
            {
